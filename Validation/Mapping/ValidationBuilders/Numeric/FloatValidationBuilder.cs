...
{
    public class FloatValidationBuilder<T>: ValidationBuilder<T>
    {
        readonly Expression<Func<T, float>> expression;

        public FloatValidationBuilder(Expression<Func<T,float>> expression, IList<IValidator<T>> validators)
            : base(validators)
        {
